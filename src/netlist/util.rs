...
pub trait NetlistUtil: NetlistBase {
    ...
    fn for_each_circuit_instance_of_net<F>(&self, net: &Self::NetId, mut f: F)
    where
...
pub trait NetlistEditUtil: NetlistEdit {
    ...
    fn flatten_circuit_instance(&mut self, circuit_instance: &Self::CellInstId) {
        ...
        assert_ne!(
            template, parent_circuit,
            "Recursive instances are not allowed."
        ); // Should not happen.
        ...
        for sub in all_instances {
            ...
            let new_name = if let (Some(sub_instance_name), Some(inst_name)) = (
                self.cell_instance_name(&sub),
                self.cell_instance_name(circuit_instance),
            ) {
                ...
                // Something like: INSTANCE_TO_BE_FLATTENED:SUB_INSTANCE{_COUNTER}
                {
                    ...
                    while self
                        .cell_instance_by_name(&parent_circuit, &new_name)
                        .is_some()
                    {
            ...
            let new_inst = self.create_cell_instance(
                &parent_circuit,
                &sub_template,
            ...
            let pin_mapping: Vec<_> = self
                .each_pin_instance(&sub)
            ...
            for (old_pin, new_pin) in pin_mapping {
                ...
                debug_assert_eq!(
                    self.template_pin(&old_pin),
                    self.template_pin(&new_pin),
        ...
        // according to the old connections to the instance which is about to be flattened.
        {
            ...
            // internal net.
            let mut net_replacement_mapping: HashMap<_, _> = self
                .each_pin_instance_vec(circuit_instance)
                .into_iter()
                .filter_map(|old_pin| {
            ...
            net_replacement_mapping
                .iter()
                .for_each(|(inner_new_net, outer_net)| self.replace_net(inner_new_net, outer_net));
            //
            // // Handle special cases: LOW and HIGH nets.
    ...
    fn purge_nets_in_circuit(&mut self, circuit_id: &Self::CellId) -> usize {
        ...
        unused.iter().for_each(|n| self.remove_net(n));
    ...
    fn create_net_names_in_circuit(&mut self, circuit_id: &Self::CellId, prefix: &str) {
        ...
        let unnamed_nets: Vec<_> = all_nets
            .iter()
